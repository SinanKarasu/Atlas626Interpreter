...
AST *	SignalTypeAST::assign	( AST * a )		{ return this; }
AST *	SignalTypeAST::check( AST * a )			{ return ASTdown()->check( a ); }
Long	SignalTypeAST::compare	( AST *  o     ) const	{ return ASTdown()->compare( o ); }
TheType	SignalTypeAST::getType	( AST * a ) const
	{
		return ASTdown()->getType( a );
	}
AST *	SignalTypeAST::Accept	( ASTVisitor & v )	{ return v.VisitSignalTypeAST( this ); }
AST *	SignalTypeAST::data(AST * a)			{ return ASTdown()->data( a ); }
...
AST *	ModifierListType::Accept	( ASTVisitor & v )	{ return v.VisitModifierListType( this ); }
...
ReqModifierListType::ReqModifierListType( ASTList * cntl,ASTList * capl,ASTList * liml )
			...
			{
				_cnt_modifierList=new ModifierListType(cntl);
				_cap_modifierList=new ModifierListType(capl);
				_lim_modifierList=new ModifierListType(liml);
				//_modifierListType=new ASTList;
				ASTListIterator cntit(*cntl);
				ASTListIterator catit(*capl);
				ASTListIterator limit(*liml);

				while(++cntit){
					_modifierList->append(cntit.key());
				}
				while(++catit){
					_modifierList->append(catit.key());
				}
				while(++limit){
					_modifierList->append(limit.key());
				}
...
AST *	ReqModifierListType::check( AST * a )	// we are the subset,a is the superset
	{
		...
		AST *result = this;
		if(cnt==NULL)result=NULL;
		if(cap==NULL)result=NULL;
		if(lim==NULL)result=NULL;
...
AST *	ReqModifierListType::Accept	( ASTVisitor & v )	{ return v.VisitReqModifierListType( this ); }
...
void
DimensionType::setDimension(AST * a)
		{
			DimensionEntry * d=a?a->getDimension():0;
			if(d)m_dimensionEntry=d;
		}
...
AST *
DimensionedNumberType::data(AST * a)			{ return this; }
...
void
DimensionedNumberType::setDimension(AST * a)
	{
		ASTdown()->ASTright()->setDimension(a);
	}

Long
DimensionedNumberType::getInteger	( int indx) const
	{
		return getDecimal();
	}
...
void
DimensionedNumberType::setInteger	( int indx,  Long   value)
	{
		setDecimal(value);
	}

void
DimensionedNumberType::setDecimal	( int indx,  double value)
	{
		ASTdown()->setDecimal(0,ScaleDimension(value,0,getDimension()));
	}

AST *
DimensionedNumberType::eval		( AST * a )
	{
		ASTdown()->eval(a);
...
AST *
DimensionedNumberType::min		( AST * a )
	{
		eval();
		return m_val_ast;
	}
...
AST *
DimensionedNumberType::max		( AST * a )
	{
		eval();
		return m_val_ast;
	}
...
AST *
DimensionedNumberType::check		( AST * a )
	{
		if(a){
			AST * loc;
...
Long
TimeQuantityType::getInteger	( int indx) const
	{
...
MeasuredCharacteristicMnemonicType::MeasuredCharacteristicMnemonicType
		( ANTLRTokenPtr p , ModifierEntry * m,RWCString & left,AtlasParser * parser)
...
void  MeasuredCharacteristicMnemonicType::print(AST * a) const
	{
		cout << " MeasuredCharacteristicMnemonicType >>" << getName() << endl;
	}

int  MeasuredCharacteristicMnemonicType::ok(RWCString & text,const RWCString test,RWCString & dest)
	{
		if(text.index(test)==0){
			dest=test;
			text.remove(0,test.length());
			return 1;
		} else {
			return 0;
		}
...
AST *
MeasuredCharacteristicMnemonicType::min		( AST * a )
	{
		return ASTdown()->min(a);
	}

AST *
MeasuredCharacteristicMnemonicType::max		( AST * a )
	{
		return ASTdown()->max(a);
	}
	

CharacteristicMnemonicType::CharacteristicMnemonicType
		( ANTLRTokenPtr p , ModifierEntry * m,RWCString & left,AtlasParser * parser )
...
void  CharacteristicMnemonicType::print(AST * a) const
	{
		cout << "CharacteristicMnemonicType Print>>" << getName() << endl;
	}
...
TedlCharacteristicMnemonicType::TedlCharacteristicMnemonicType
		( ANTLRTokenPtr p , ModifierEntry * m,RWCString & left,AtlasParser * parser )
...
TheType	MeasuredCharacteristicType::getType	( AST * a ) const	{ return ASTdown()->getType( a ); }
...
void  SignalValueType::print(AST * a) const
	{
		cout << " SignalValueType >>" << getName() << endl;
	}
...
void  EventSlopePosType::print(AST * a) const
	{
		cout << " EventSlopePosType >>" << getName() << endl;
	}
...
void  EventSlopeNegType::print(AST * a) const
	{
		cout << " EventSlopeNegType >>" << getName() << endl;
	}
...
void  HysteresisType::print(AST * a) const
	{
		cout << " HysteresisType >>" << getName() << endl;
	}
